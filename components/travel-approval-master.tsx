...
const initialTravelApprovals: TravelApprovalRow[] = [];

function TravelApprovalForm({ row, onSave, onBack }: { row: any; onSave: (r: TravelApprovalRow) => void; onBack: () => void }) {
  const [form, setForm] = useState<TravelApprovalRow>({
    id: row.id ?? "",
    ...
    <section className="subdivision-console">
      <div className="subdivision-head">
        <div>
          ...
          <h2>{isEdit ? "Edit Travel Claim" : "Add Travel Claim"}</h2>
      ...
      <div className="subdivision-form-card">
        ...
        <label className="field">
          <span>* Claim ID</span>
          <input value={form.claimId} onChange={e => setForm({ ...form, claimId: e.target.value })} placeholder="e.g. CLM10293" />
        </label>
        <label className="field">
          <span>* Amount</span>
          <input type="number" value={form.amount || ""} onChange={e => setForm({ ...form, amount: parseFloat(e.target.value) || 0 })} placeholder="e.g. 1500" />
        </label>
        <label className="field">
          <span>Status</span>
...
export function TravelApprovalMaster() {
  const [list, setList] = useState<TravelApprovalRow[]>(initialTravelApprovals);
  const [search, setSearch] = useState("");
  const [view, setView] = useState<"list" | "add" | "edit">("list");
  const [editTarget, setEditTarget] = useState<TravelApprovalRow | null>(null);
  ...
  function handleSave(form: TravelApprovalRow) {
    if (view === "add") {
      const newRow = {
        ...form,
        id: `TRV${String(list.length + 1).padStart(3, "0")}`
      };
      setList([...list, newRow]);
    } else {
  ...
  if (view === "add") return <TravelApprovalForm row={{}} onSave={handleSave} onBack={() => setView("list")} />;
  if (view === "edit" && editTarget) return <TravelApprovalForm row={editTarget} onSave={handleSave} onBack={() => setView("list")} />;
